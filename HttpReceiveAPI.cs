...
{
    public static class HttpReceiveAPI
    {
        ...
        private static readonly int cacheExpire = FunctionUtilities.GetIntValue(Environment.GetEnvironmentVariable("CacheTimeout"), 60);
        ...
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "POST", Route = null)] HttpRequest req, ILogger log)
        {
            ...
            var telemetryClient = Helper.GetTelemetryClient($"{RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API}", InstrumentationKey, out operation);
            //
            try
            {
                if (string.IsNullOrWhiteSpace(tenantId))
                {
                    telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"Tenant ID should be defined in Function APP Configurations!");
                ...
                var orderXml = await req.ReadAsStringAsync();

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, "Starting...");

                var tenant = TenantTableHelper.GetTenantModel(commonStorageConnetionString, tenantId, cacheExpire, telemetryClient);

                if (tenant == null)
                {
                    telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"Tenant {tenantId} is not configured!");
                ...
                var processName = FunctionHelper.GetKeyIdentifierFromXML(orderXml, processNameXmlPath);

                if (string.IsNullOrWhiteSpace(processName))
                {
                    telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"ProcessName value is required!");
                ...
                var processflow = FunctionHelper.GetProcessFlowByName(processName, tenant.CloudDb);

                var stepParams = processflow.Functions.Where(f => f.StepOrder == 1).FirstOrDefault();

                var partnerShip = PartnershipDbHelper.GetPartnershipByPartnershipId(stepParams.PartnershipId, tenant.CloudDb).ToObject<List<PartnerShip>>().FirstOrDefault();

                var apiParams = FunctionHelper.GetApiParams(stepParams, partnerShip);

                var stage = FunctionHelper.EdiStageObject(apiParams);

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"Uploading Original message to blob storage.");

                var blobPath = await tenant.AzureBlob.UploadFileToBlobStorageAsync(orderXml, $"{apiParams.PartnershipCode}/{stage.Transaction_Id}/{apiParams.TransactionStep}.txt", ArchiveBlobContainer);

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, "Pushing Success Message.");

                string keyIdentifier = "";

                if (!string.IsNullOrEmpty(apiParams.KeyIdentifier))
                {
                    keyIdentifier = FunctionHelper.GetKeyIdentifierFromXML(orderXml, apiParams.KeyIdentifier);
                }

                stage.ISAControlNum = keyIdentifier;

                stage.id = FunctionHelper.LogSuccessStageMessage(tenant.CloudDb, stage, apiParams, blobPath);

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, "Pushed Success Message.");

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"Pushing message in Topic {apiParams.TargetTopic}.");
                ...
                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, "Pushed EDI message in Topic.");

                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, "Finished.");
            ...
            {
                telemetryClient.TrackTrace(RCKFunctionNames.FUNC_RCK_ECOM_ORDER_API, $"an error occurres {ex}");
